...
const ManageIngredients = () => {
  ...
  // Nút Submit dùng chung cho Tạo mới và Cập nhật
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Đã thêm mới', showConfirmButton: false, timer: 1500 });
      }
  ...
  const handleDelete = (id, name) => {
    Swal.fire({
      title: `Xóa "${name}"?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Xóa ngay'
    }).then(async (result) => {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '30px', background: '#fff', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
      <h2 style={{ textAlign: 'center', color: '#ff6b00', marginBottom: '30px' }}>🛒 Quản lý Kho Nguyên liệu</h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '15px', alignItems: 'flex-start', marginBottom: '40px', background: '#f9f9f9', padding: '20px', borderRadius: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: '250px' }}>
            <label style={{ display:'block', marginBottom:'8px', fontWeight:'bold' }}>Tên nguyên liệu</label>
            <input type="text" placeholder="Ví dụ: Thịt bò" value={name} onChange={(e) => setName(e.target.value)} required style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
        </div>

        <div style={{ flex: 1, minWidth: '250px' }}>
            <label style={{ display:'block', marginBottom:'8px', fontWeight:'bold' }}>Hình ảnh minh họa</label>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignSelf: 'center', marginTop: '20px' }}>
            ...
            {editingId && (
                <button type="button" onClick={cancelEdit} style={{ padding: '8px 25px', background: '#ccc', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Hủy</button>
            )}
        </div>
      </form>
      ...
      <h4 style={{ color: '#333', borderBottom: '2px solid #eee', paddingBottom: '10px', marginBottom: '20px' }}>
        Danh sách hiện có ({ingredients.length})
      </h4>
          ...
          <div key={ing._id} style={{ display: 'flex', flexDirection: 'column', background: '#fff', border: '1px solid #eee', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <div style={{ height: '120px', backgroundColor: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {ing.image ? (
                    ...
                    <span style={{ color: '#aaa', fontSize: '12px' }}>Chưa có ảnh</span>
                )}
            </div>
            <div style={{ padding: '15px', textAlign: 'center' }}>
                <h4 style={{ margin: '0 0 15px', color: '#333' }}>{ing.name}</h4>
                <div style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
                    <button onClick={() => startEdit(ing)} style={{ padding: '5px 15px', background: '#e9ecef', border: 'none', borderRadius: '4px', cursor: 'pointer', color: '#333' }}>Sửa</button>
                    <button onClick={() => handleDelete(ing._id, ing.name)} style={{ padding: '5px 15px', background: '#ffeded', border: 'none', borderRadius: '4px', cursor: 'pointer', color: '#dc3545' }}>Xóa</button>
                </div>
            </div>
          </div>
        ))}
